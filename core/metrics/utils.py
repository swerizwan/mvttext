...
def calculate_frechet_distance_np(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    return diff.dot(diff) - diff_f + np.trace(sigma1) + np.trace(
        sigma2) - 2 * tr_covmean

def calculate_gt_frechet_distance_np(mu1, sigma1, mu2, sigma2, eps=1e-6):
    """Numpy implementation of the Frechet Distance.
    The Frechet distance between two multivariate Gaussians X_1 ~ N(mu_1, C_1)
    and X_2 ~ N(mu_2, C_2) is
            d^2 = ||mu_1 - mu_2||^2 + Tr(C_1 + C_2 - 2*sqrt(C_1*C_2)).
...
def calculate_diversity_multimodality(activations,
    ...
    while np.any(label_quotas > 0):
        ...
        multimodality = multi_vara / multi_varb
...
def calculate_gt_diversity_multimodality(activations,
                                      labels,
                                      num_labels,
                                      diversity_times=200,
                                      multimodality_times=20):
...
def calculate_gt_fid(statistics_1, statistics_2):
    return calculate_gt_frechet_distance_np(statistics_1[0], statistics_1[1],
                                         statistics_2[0], statistics_2[1])
