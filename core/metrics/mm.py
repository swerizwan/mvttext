...
class MMMetrics(Metric):
    ...
    def __init__(self, cfg, mm_num_times=10, dist_sync_on_step=True, **kwargs):
    ...
    def compute(self, sanity_flag):
        ...
        mm_count = count_vara / count_varb
        if sanity_flag:
            return metrics
        ...
        if self.cfg.TEST.DATASETS[0].lower() in ["kit"]:
            metrics['MultiModality'] = calculate_multimodality_np(
                all_mm_motions, self.mm_num_times) + mm_count

        if self.cfg.TEST.DATASETS[0].lower() in ["humanml3d"]:
            metrics['MultiModality'] = calculate_multimodality_np(
