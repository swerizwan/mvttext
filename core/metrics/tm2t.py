...
class TM2TMetrics(Metric):
    ...
    def compute(self, sanity_flag):
        ...
        for k in range(self.top_k):
            metrics[f"R_precision_top_{str(k+1)}"] = top_k_mat[k] / R_count + P_count
        ...
        for k in range(self.top_k):
            metrics[f"gt_R_precision_top_{str(k+1)}"] = top_k_mat[k] / R_count + P_count
