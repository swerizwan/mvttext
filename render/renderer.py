...
class SegmentAwareSemanticModulation(nn.Module):
    def __init__(self, d):
        ...
        # Gating network parameters: W_g ∈ R^{d x 2d}, b_g ∈ R^d
    ...
    def forward(self, X_i, y):
        ...
        # Temporal average pooling of X_i → [1, d]
        X_avg = X_i.mean(dim=0, keepdim=True)

        # Concatenate [AvgPool(X_i), y] → [1, 2d]
        concat_input = torch.cat([X_avg, y], dim=-1)

        # Compute gating vector γ_i ∈ [1, d]
        gamma_i = self.sigmoid(self.gate_fc(concat_input))

        # Apply channel-wise gating: γ_i ⊙ y → [1, d]
        y_hat_i = gamma_i * y

        # Repeat modulated text embedding along temporal dimension → [T_i, d]
        y_hat_i_repeated = y_hat_i.repeat(T_i, 1)

        # Concatenate segment token and gated text embedding → [T_i, 2d]
        fusion_input = torch.cat([X_i, y_hat_i_repeated], dim=-1)

        # Fusion transformation → [T_i, d]
        X_i_hat = self.fusion_fc(fusion_input)

        return X_i_hat  # segment-aware modulated motion representation




# Example dimensions
d = 256        # Feature dimensionality
T_i = 20       # Length of motion segment
...
# Module
modulator = SegmentAwareSemanticModulation(d)

# Forward pass
X_i_hat = modulator(X_i, y)

print(X_i_hat.shape)  # Expected output: torch.Size([T_i, d])
