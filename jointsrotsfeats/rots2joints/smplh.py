...
class SMPLH(Rots2Joints):
    ...
    def forward(self,
        ...
        # from tools.easyconvert import to_matrix
...
def smplh_to(jointstype, data, trans):
    from utils.joints import get_root_idx

    if "mmm" in jointstype:
        from utils.joints import smplh2mmm_indexes
        indexes = smplh2mmm_indexes
        data = data[..., indexes, :]

        # make it compatible with mmm
        if jointstype == "mmm":
            from utils.joints import smplh_to_mmm_scaling_factor
        ...
        from utils.joints import smplh2smplnh_indexes
