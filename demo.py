...
from data.sampling import subsample, upsample
from core.get_model import get_model
...
def main():
    ...
    if cfg.DEMO.EXAMPLE:
        ...
        # load txt
        from utils.demo_utils import load_example_input
        ...
        # motion transfer
            ...
            features = model.jointsrotsfeats.joints2jfeats(joints_sample[None])
            motion = xx
            # datastruct = model.jointsrotsfeats.Datastruct(features=features).to(model.device)
    ...
    with torch.no_grad():
        ...
        if text:
            ...
            for rep in range(cfg.DEMO.REPLICATION):
                ...
                # cal inference time
                infer_time = time.time() - motiontext_time
                num_batch = 1
